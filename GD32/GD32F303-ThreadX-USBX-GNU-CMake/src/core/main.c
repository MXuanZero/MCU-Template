...
UCHAR device_framework_full_speed[] = {
	...
	/* Device descriptor */
	0x12, 0x01, 0x10, 0x01, 0x00, 0x00, 0x00, 0x08, 0xec, 0x08, 0x10, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x01,
...
UCHAR device_framework_high_speed[] = {
	...
	/* Device descriptor */
	0x12, 0x01, 0x00, 0x02, 0x00, 0x00, 0x00, 0x40, 0x0a, 0x07, 0x25, 0x40, 0x01, 0x00, 0x01,
	0x02, 0x03, 0x01,
...
UCHAR string_framework[] = {
	...
	0x09, 0x04, 0x01, 0x0c, 0x45, 0x78, 0x70, 0x72, 0x65, 0x73, 0x20, 0x4c, 0x6f, 0x67, 0x69,
	0x63,

	/* Product string descriptor : Index 2 */
	0x09, 0x04, 0x02, 0x0c, 0x44, 0x61, 0x74, 0x61, 0x50, 0x75, 0x6d, 0x70, 0x44, 0x65, 0x6d,
	0x6f,
...
void tx_application_define(void *first_unused_memory)
{
	...
	status = ux_host_stack_class_register(_ux_system_host_class_dpump_name,
	...
	status = _ux_device_stack_class_register(_ux_system_slave_class_dpump_name,
						 _ux_device_class_dpump_entry, 1, 0, &parameter);
	...
	status = tx_thread_create(&tx_demo_thread_host_simulation, "tx demo host simulation",
				  tx_demo_thread_host_simulation_entry, 0, stack_pointer,
	...
	status = tx_thread_create(&tx_demo_thread_device_simulation, "tx demo slave simulation",
				  tx_demo_thread_device_simulation_entry, 0,
				  stack_pointer + UX_DEMO_STACK_SIZE, UX_DEMO_STACK_SIZE, 20, 20, 1,
				  TX_AUTO_START);
...
void tx_demo_thread_host_simulation_entry(ULONG arg)
{
	...
	while (1) {
		...
		_ux_utility_memory_set(host_out_buffer, current_char,
				       UX_HOST_CLASS_DPUMP_PACKET_SIZE); /* Use case of memset is
									    verified. */
		...
		status = _ux_host_class_dpump_write(
			dpump, host_out_buffer, UX_HOST_CLASS_DPUMP_PACKET_SIZE, &actual_length);
		...
		status = _ux_host_class_dpump_read(dpump, host_in_buffer,
...
void tx_demo_thread_device_simulation_entry(ULONG arg)
{
	...
	while (1) {
		...
		while (dpump_device != UX_NULL) {
			...
			status = _ux_device_class_dpump_read(dpump_device, device_buffer,
							     UX_HOST_CLASS_DPUMP_PACKET_SIZE,
							     &actual_length);

			/* Verify that the status and the amount of data is correct.  */
			if ((status != UX_SUCCESS) ||
			    actual_length != UX_HOST_CLASS_DPUMP_PACKET_SIZE)
				error_handler();

			/* Now write to the device data pump.  */
			status = _ux_device_class_dpump_write(dpump_device, device_buffer,
							      UX_HOST_CLASS_DPUMP_PACKET_SIZE,
							      &actual_length);
